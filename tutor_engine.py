...
class PedagogicalTracker:
    ...
    def start_query_session(
        ...
        if "chapter_name" not in concept_data:
            concept_data["chapter_name"] = top_match.get("chapter_name", "NCERT Biology")
        if "topic" not in concept_data:
            concept_data["topic"] = top_match.get("topic", top_match.get("section", "Biology Concept"))
        if "chapter_id" not in concept_data:
            concept_data["chapter_id"] = top_match.get("chapter_id", "c01")
    ...
    def get_step_content(self, session_id: str, step_number: int) -> Dict[str, Any]:
        ...
        title = data.get("title", "NCERT Biology Concept")
        chap = data.get("chapter_name") or data.get("chapter_title", "NCERT Biology")
