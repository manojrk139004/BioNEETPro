...
class NCERTIngestionPipeline:
    ...
    def process_chapter_pdf(self, pdf_path: Path) -> List[Dict[str, Any]]:
        ...
        def flush_chunk():
            ...
            # Full class tag (ClassXI vs ClassXII): truncated prefixes collided
            # across classes (both 'ClassXI'), duplicating chunk IDs.
            class_tag = class_name.replace(" ", "")
            chunk_id = f"NCERT-{class_tag}-CH{chapter_num:02d}-P{avg_page:03d}-{len(chunks)+1:02d}"
